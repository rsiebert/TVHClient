...
public class ProgramGuideViewPagerFragment extends Fragment implements ProgramGuideControlInterface, ProgramGuideScrollInterface, ChannelTagSelectionCallback, SearchRequestInterface {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        Timber.d("onActivityCreated() called with: savedInstanceState = [" + savedInstanceState + "]");
        activity = getActivity();
        menuUtils = new MenuUtils(getActivity());

        // Calculate the max number of fragments in the view pager
        Timber.d("onActivityCreated: calcFragmentCount");
        calcFragmentCount();
        Timber.d("onActivityCreated: calcFragmentCount done");
        Timber.d("onActivityCreated: calcProgramGuideTimeslots");
        calcProgramGuideTimeslots();
        Timber.d("onActivityCreated: calcProgramGuideTimeslots done");
        ...
        if (savedInstanceState == null) {
            Timber.d("onActivityCreated: new channel list fragment");
        ...
        Timber.d("onActivityCreated: new adapter");
        adapter = new ProgramGuideViewPagerAdapter(getChildFragmentManager(), fragmentCount);
        viewPager.setAdapter(adapter);
        viewPager.setOffscreenPageLimit(1);
        Timber.d("onActivityCreated: adapter done");
        adapter.notifyDataSetChanged();
        Timber.d("onActivityCreated: notify done");
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        final boolean showGenreColors = prefs.getBoolean("genre_colors_for_channels_enabled", false);
        (menu.findItem(R.id.menu_genre_color_info_channels)).setVisible(showGenreColors);

        (menu.findItem(R.id.menu_timeframe)).setVisible(MainApplication.getInstance().isUnlocked());

        // Prevent the channelTextView tag menu item from going into the overlay menu
        if (prefs.getBoolean("channel_tag_menu_enabled", true)) {
