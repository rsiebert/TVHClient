...
public class PlayActivity extends Activity implements OnRequestPermissionsResultCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ch != null) {
            ...
            Timber.d("onCreate: No channel or recording provided, exiting");
            return;
        }
    ...
    private void initAction() {
        ...
        case ACTION_PLAY:
            ...
            if (rec != null && app.isUnlocked()) {
                ...
                Timber.d("initAction: Downloaded recording can be played from '" + file.getAbsolutePath()  + "': " + file.exists());
                if (file.exists()) {
        ...
        case ACTION_CAST:
            if (ch != null && ch.getNumber() > 0) {
                Timber.d("initAction: Starting to cast channel '" + ch.getName() + "'");
                startCasting();
            } else if (rec != null) {
                Timber.d("initAction: Starting to cast recording '" + rec.getTitle() + "'");
                startCasting();
            }
            break;
        }
        Timber.d("initAction() returned");
    }
    ...
    private void initPlayback(String path, String ticket) {
        Timber.d("getIntentForExternalPlayerback() called with: path = [" + path + "], ticket = [" + ticket + "]");
/*
        ...
        if (playbackProfile == null) {
            Timber.d("getIntentForExternalPlayerback: no profile defined, creating default profile");
        ...
        Timber.d("getIntentForExternalPlayerback() returned");
    ...
    private void startPlayback(String url, String mime) {
        ...
        Timber.d("startPlayback() called with: url = [" + logUrl + "], mime = [" + mime + "]");
        ...
        this.runOnUiThread(new Runnable() {
            public void run() {
                try {
                    Timber.d("startPlayback: Starting external player");
                    startActivity(playbackIntent);
                    finish();
                } catch (Throwable t) {
                    Timber.d("startPlayback: Can't execute external media player");
                    ...
                    new MaterialDialog.Builder(PlayActivity.this)
                        ...
                        .onPositive(new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                try {
                                    Timber.d("startPlayback: Starting play store to download external players");
                                    Intent installIntent = new Intent(Intent.ACTION_VIEW);
                                    installIntent.setData(Uri.parse("market://search?q=free%20video%20player&c=apps"));
                                    startActivity(installIntent);
                                } catch (Throwable t2) {
                                    Timber.d("startPlayback: Could not start google play store");
                                } finally {
                                    finish();
                                }
                            }
        ...
        Timber.d("startPlayback() returned");
    }
    ...
    private void startCasting() {
        ...
        if (ch != null) {
            castUrl += "/stream/channelnumber/" + ch.getNumber();
            iconUrl += "/" + ch.getIcon();
        ...
        Timber.d("startCasting: Casting the following program: title [" + title + "], url [" + castUrl + "]");

        RemoteMediaClient remoteMediaClient = castSession.getRemoteMediaClient();
        remoteMediaClient.load(mediaInfo, true, 0);

        Timber.d("startCasting() returned");
        finish();
    }
