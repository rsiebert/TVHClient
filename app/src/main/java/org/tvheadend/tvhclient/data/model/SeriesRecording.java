...
public class SeriesRecording {
    ...
    private int enabled;         // u32   required   If autorec entry is enabled (activated).
    private String name;         // str   required   Name of the autorec entry (Added in version 18).
    @ColumnInfo(name = "min_duration")
    private int minDuration;     // u32   required   Minimal duration in seconds (0 = Any).
    @ColumnInfo(name = "max_duration")
    private int maxDuration;     // u32   required   Maximal duration in seconds (0 = Any).
    private int retention;       // u32   required   Retention time (in days).
    @ColumnInfo(name = "days_of_week")
    private int daysOfWeek;      // u32   required   Bitmask - Days of week (0x01 = Monday, 0x40 = Sunday, 0x7f = Whole Week, 0 = Not set).
    private int priority;        // u32   required   Priority (0 = Important, 1 = High, 2 = Normal, 3 = Low, 4 = Unimportant, 5 = Not set).
    @ColumnInfo(name = "approx_time")
    private int approxTime;      // u32   required   Minutes from midnight (up to 24*60).
    private long start;          // s32   required   Exact start time (minutes from midnight) (Added in version 18).
    @ColumnInfo(name = "start_window")
    private long startWindow;    // s32   required   Exact stop time (minutes from midnight) (Added in version 18).
    @ColumnInfo(name = "start_extra")
    private long startExtra;     // s64   required   Extra start minutes (pre-time).
    @ColumnInfo(name = "stop_extra")
    private long stopExtra;      // s64   required   Extra stop minutes (post-time).
    ...
    private int channelId;         // u32   optional   Channel ID.
