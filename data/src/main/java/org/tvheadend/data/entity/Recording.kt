...
data class Recording(
        ...
        var start: Long = Calendar.getInstance().timeInMillis,                     // s64   required   Time of when this entry was scheduled to start recording.
        var stop: Long = Calendar.getInstance().timeInMillis + (30 * 60 * 1000).toLong(),             // s64   required   Time of when this entry was scheduled to stop recording.
...
internal data class RecordingEntity(
    ...
    companion object {
        fun from(recording: Recording): RecordingEntity {
            return RecordingEntity(
                    ...
                    recording.owner,
                    recording.creator,
                    recording.subscriptionError,
                    recording.streamErrors,
                    recording.dataErrors,
                    recording.path,
                    recording.dataSize,
                    recording.isEnabled,
                    recording.duplicate,
                    ...
                    recording.comment,
                    recording.image,
                    recording.fanartImage,
                    recording.copyrightYear,
                    recording.removal,
                    recording.files,
    ...
    fun toRecording(): Recording {
        return Recording(id, channelId, start, stop, startExtra, stopExtra, retention, priority, eventId, autorecId, timerecId, contentType, title, subtitle, summary, description, state, error, owner, creator, subscriptionError, streamErrors, dataErrors, path, dataSize, isEnabled, duplicate, episode, comment, image, fanartImage, copyrightYear, removal, files, connectionId, channelName, channelIcon, duration)
    }
}
